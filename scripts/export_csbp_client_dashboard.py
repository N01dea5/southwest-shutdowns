#!/usr/bin/env python3
"""Export a sanitised CSBP NAAN2 client feed.

The internal Southwest dashboard remains the source of truth. This script reads
internal generated JSON/history, selects the CSBP NAAN2 June 2026 shutdown,
strips sensitive fields, and emits a stable client-facing feed.
...
OUT_FILE = OUT_DIR / "csbp-naan2-2026.json"
...
def _role_key(role: str) -> str:
    ...
    # Keep the client-facing CSBP role naming aligned to the historical client dashboard.
    return {
        "Supervisor - Mechanical": "Supervisor",
        "Mechanical Fitter": "Fitter",
        "Advanced Rigger": "Rigger",
        "Intermediate Rigger": "Rigger",
    }.get(role, role)


def _find_target_shutdown() -> dict[str, Any]:
    csbp = _load_json(DATA_DIR / "csbp.json")
    for shutdown in csbp.get("shutdowns", []):
    ...
    for company, shutdown in _all_shutdowns():
        ...
        for worker in shutdown.get("roster", []) or []:
            ...
            if not key:
                continue
            seen_any.add(key)
            if company.lower() == "csbp" or str(shutdown.get("id", "")).startswith("csbp"):
                seen_csbp.add(key)
    return seen_any, seen_csbp
...
def _summarise_roles(map_in: dict[str, Any]) -> dict[str, int]:
    out: dict[str, int] = {}
    for role, value in (map_in or {}).items():
        key = _role_key(str(role))
...
def _build_feed() -> dict[str, Any]:
    ...
    seen_any, seen_csbp = _prior_worker_sets(target_start)
    ...
    required = _summarise_roles(shutdown.get("required_by_role") or {})
    filled = _summarise_roles(shutdown.get("filled_by_role") or {})
    ...
    feed = {
        ...
        "source_of_truth": "N01dea5/southwest-shutdowns:data/history/csbp-2026-05.json",
        "client": "CSBP",
        "dashboard": "CSBP NAAN2 Shutdown Workforce Dashboard",
        "shutdown": {
            "id": shutdown.get("id"),
...
def main() -> int:
    ...
    print(f"export_csbp_client_dashboard: wrote {OUT_FILE.relative_to(REPO_ROOT)}")
    return 0
